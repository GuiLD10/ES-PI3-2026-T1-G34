...
import {
  listStartupTransactions as listTransactions,
...
export const listStartupTransactions = onRequest(async (req, res) => {
  ...
  try {
    ...
    const data = await listTransactions(startupId);
    ...
    console.error("Erro ao listar transacoes da startup:", error);
    const response = buildExchangeErrorResponse(
      error,
      "Erro interno ao listar transacoes. Tente novamente.",
    );
